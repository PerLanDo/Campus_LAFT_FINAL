...
interface ItemDetailActionsProps {
  item: Item;
  user: User | null; // Changed from Profile to User
  isOwner: boolean;
  itemUrl: string;
}
...
export default function ItemDetailActions({
  ...
  const handleClaim = async () => {
    ...
    const { data: inserted, error } = await supabase
      ...
      .insert({
        ...
        // security turn-in now managed by admins; no user input
      })
      .select();
    if (error) {
      ...
      if (insertedClaim) {
        await supabase.from("notifications").insert({
          ...
          message: `${
            user?.user_metadata?.full_name || user?.email || "A user"
  ...
  return (
    <>
      {/* Main content area with flex or grid for potential side-by-side layout */}
      <div className="flex flex-col lg:flex-row gap-6">

        {/* Left Column: Actions and Status */}
        <div className="flex-1">
          {/* Notification for Pending Claims (Owner only) */}
          ...
          {/* User's Claim Status (Non-owner, if claimed) */}
          {!isOwner && userClaim ? (
            <div className="w-full bg-gray-800 rounded-xl border border-gray-700 shadow-lg overflow-hidden mb-6">
              ...
              <div className="p-4">
                ...
                {/* Chat Button for rejected claims */}
                {userClaim.status === "rejected" && (
                  <div className="mt-4">
                    <ChatInitiator itemId={item.id} />
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-end">
              {isOwner ? (
                <>
                  ...
                  {/* Delete Button */}
                  <button
                    className="flex-1 sm:flex-auto bg-gradient-to-r from-red-600 to-red-500 hover:from-red-700 hover:to-red-600 text-white font-bold py-3 px-6 rounded-lg shadow-lg transition-all duration-200 flex items-center justify-center gap-2"
                  ...
                  {/* Chat with Security Button (Owner) */}
                  {userRole !== "admin" && (
                    <div className="flex-1 sm:flex-auto">
                      <ChatInitiator itemId={item.id} turnInToSecurity={true} />
                    </div>
                  )}
                ...
                <>
                  {/* Claim Button (Non-owner, if not claimed) */}
                  {!userClaim && (
                    <button
                      className="flex-1 sm:flex-auto bg-gradient-to-r from-indigo-600 to-indigo-500 hover:from-indigo-700 hover:to-indigo-600 text-white font-bold py-3 px-8 rounded-lg shadow-lg transition-all duration-200 flex items-center justify-center gap-2"
                  ...
                  {/* Chat Button (Non-owner) */}
                  {user && !isOwner && (
                    <div className="flex-1 sm:flex-auto">
                      <ChatInitiator itemId={item.id} turnInToSecurity={item.turn_in_to_security} />
                    </div>
                  )}
                </>
              )}
            </div>
          </div>{/* End of Action Buttons Section Card */}
        </div>{/* End of Left Column */}

        {/* Right Column: QR Code */}
        <div className="lg:w-1/3 flex justify-center lg:justify-start">
          {/* Enhanced QR Code Presentation */}
          <div className="bg-gradient-to-r from-gray-800 to-gray-900 rounded-xl border border-gray-700 shadow-lg p-5 flex flex-col items-center max-w-xs lg:max-w-full w-full">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4">
          <div className="bg-gradient-to-r from-gray-800 to-gray-900 border border-gray-700 rounded-xl shadow-2xl max-w-md w-full animate-fadeIn">
            ...
            <div className="p-5">
              ...
              />
              {/* Informational note for campus security handover */}
              <p className="text-gray-400 text-sm mb-4">
                If this item has already been turned over to campus
                security, please claim it at the campus security building.
              </p>
              {claimError && (
